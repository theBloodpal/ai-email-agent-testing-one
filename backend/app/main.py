...
def _db():
    ...
    if _DB_CONN is None:
        ...
        _DB_CONN = sqlite3.connect("users.db", check_same_thread=False)
...
def _fetch_inbox_snippets(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Could not connect to your email inbox. Please verify your Email and App Password. (Error: {str(e)})"
        )
    if isinstance(inbox_cache, dict):
...
def _send_worker_job(job_id: str, subject: str, message_template: str, snapshot: dict[str, Any]) -> None:
    ...
    try:
        if imap_user and imap_pass:
            ...
            bounces = check_bounces()
        bounced_emails = {
            str(b["email"]).lower().strip() for b in (bounces or []) if b.get("email")
        }
        if bounced_emails:
            with _send_jobs_lock:
                p = state["send_jobs"][job_id]["progress"]
                for r in p["results"]:
                    ...
                    if r.get("status") == "delivered" and em and em in bounced_emails:
                        r["status"] = "bounced"
...
async def send_status() -> dict[str, object]:
    ...
    return {
        ...
        "progress": dict(state.get("send_progress") or {}),
        "jobs": jobs,
